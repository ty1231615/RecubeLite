...
class FirstDifficultySession(StepWithLevelSession):
    ...
    """
    def __init__(self, surface: Surface, stage: Stage, stageLevel: int, maxStageLevel: int, players: list[Player], enemys: list[ComputeEnemy], view: SessionDesignView, addEnemys:list[ComputeEnemy], levelStepProgress: Progress) -> None:
        super().__init__(surface, stage, stageLevel, maxStageLevel, players, enemys, view, levelStepProgress, Progress(0, len(addEnemys)-1, 0, 1))
